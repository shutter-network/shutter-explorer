...
const Validator = () => {
  const {
    data: shutterizedValidatorsData,
    loading: loadingShutterized,
    error: errorShutterized,
  } = useFetch(`/api/validator/total_registered_validators`);
  const {
    data: totalValidatorsData,
    loading: loadingTotal,
    error: errorTotal,
  } = useFetch("/api/validator/total_gnosis_validators");

  const [shutterizedValidators, setShutterizedValidators] = useState(
    shutterizedValidatorsData?.message || "N/A"
  );
  const [validatorPercentage, setValidatorPercentage] = useState(
    (shutterizedValidatorsData?.message * 100) / totalValidatorsData?.message ||
      "N/A"
  );
  const [totalValidators, setTotalValidators] = useState(
    totalValidatorsData?.message || "N/A"
  );
    ...
    if (socket) {
      socket.onmessage = (event: MessageEvent) => {
        ...
        if (websocketEvent.error) {
          setWebSocketError(
            `Error: ${websocketEvent.error.message} (Code: ${websocketEvent.error.code})`
          );
          ...
          switch (websocketEvent.type) {
            case "shutterized_validators_updated":
              if ("count" in websocketEvent.data) {
                ...
                setValidatorPercentage(
                  (websocketEvent.data.count * 100) / totalValidators
                );
              } else {
                console.warn(
                  "Invalid data format for shutterized_validators_updated"
                );
              }
              break;
            case "total_validators_updated":
              if ("count" in websocketEvent.data) {
                ...
                setValidatorPercentage(
                  (shutterizedValidators * 100) / websocketEvent.data.count
                );
              } else {
                console.warn(
                  "Invalid data format for total_validators_updated"
                );
              }
              break;
            default:
          ...
          setWebSocketError(
            `Received null data for event type: ${websocketEvent.type}`
          );
          console.warn(
            "Received null data for event type:",
            websocketEvent.type
          );
        }
      };
      ...
      <OverviewCard title="Validator Overview" iconSrc={overviewIcon}>
        ...
        {errorShutterized ? (
          <Alert severity="error">
            Error fetching shutterized validators: {errorShutterized.message}
          </Alert>
        ) : (
          <InfoBox
        ...
        {errorTotal ? (
          <Alert severity="error">
            Error fetching total validators: {errorTotal.message}
          </Alert>
        ) : (
          <InfoBox
            title="# Validators"
